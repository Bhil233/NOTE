...
### 电感连续电流工作模式
$$
U_L = \frac{(U_i-U_o)t_{on}-U_ot_{off}}{T_a}
$$
$$
\frac{U_o}{U_i}=D
$$
...
### 电感电流连续工作模式
$$
U_L=\frac{U_it_{on}+U_ot_{off}}{T_a}
$$
$$
...
升降压电路的输出电压与输入电压极性相反（实际经常忽略其极性，采用绝对值表示），其数值既可以高于输入电压也可以低于输入电压。

## 丘克（Cuk）斩波电路
Cuk电路可以看作由升压斩波电路和降压斩波电路前后级联形成的。
$$
\frac{U_o}{U_i}=-\frac{D}{1-D}
$$
应用于对输入输出纹波要求较高反相型开关稳压器。

## 双向DC-DC交流电路
### 二象限斩波电路
$$
\frac{U_o}{U_i}=\frac{t_{omS1}}{T}=D_{S1}
$$
$$D_1 = 1-D_2$$
式中$t_{onS1}$是$S_1$的导通时间，*T*是开关周期，$D_{S1}$是$S_1$的占空比
当输出电流为正时，应控制开关管$V_1$的占空比调节输出电流。
当输出电流为负时，应控制开关管$V_2$的占空比调节输出电流。

二象限斩波电路可以灵活、快速地控制负载电流，适用于需要电能双向传输，但又不需要改变输出电压极性的场合，如蓄电池充放电电源、直流电机不可逆调速装置等。
### 四象限斩波电路
四象限斩波电路可用于既需要电能双向传输，又需要改变输出电压极性的场合，如直流电机可逆调速装置等。

# 第六章（AC-AC）
## 单相交流调压电路
### 电阻负载
在触发角为α时，负载电压有效值$U_o$、负载电流有效值$I_o$、晶闸管电流有效值$I_{VT}$和电流功率因素λ分别为
$$
U_o = \sqrt{\frac{1}{\pi}\int_\alpha^\pi(\sqrt2U_1\sin\omega t)^2d(\omega t)} = U_1\sqrt{\frac{1}{2\pi}\sin2\alpha+\frac{\pi-\alpha}{\pi}}
$$
$$
I_o = \frac{U_o}{R}
$$
...
$$
## 交流调功电路
交流调功电路和交流调压电路的电路形式完全相同，只是控制方式不同。交流调功电路不是在每个交流电周期都通过触发角α对输出电压波形进行控制，而是将负载于交流电源接通几个整周波，再断开几个整周波，通过改变接通周波数于断开周波数的比值来调节负载所消耗的平均功率。

控制对象一般为电炉等时间常数较大的控制对象，不需要对交流电源整个周期进行频繁的控制。不对电网电压电流造成通常意义的谐波污染。

## 交流电力电子开关
和机械开关相比，这种开关响应速度快，没有触电，寿命长，可以频繁控制通断。
...
交-交变频电路主要用于1000KW以上的大功率、低转速的交流调速电路中。目前已在轧机主传动装置、鼓风机、矿石粉碎机、球磨机、卷扬机等场合获得了较多应用，既可用于异步电动机传动，也可用于同步电动机传动。

# 第七章(PWM)
面积等效法则：冲量相等而形状不同的窄脉冲加在具有惯性的环节上时，其效果基本相同。

SPWM，正弦脉冲宽度调制，是输入一段幅值相等的脉冲序列去等效正弦波。

**自然采样法**
自然采样法是用需要调制的正弦波与载波锯齿波的交点，来确定最终PWM脉冲所需要输出的时间宽度，最终由此生成SPWM波。
**规则采样法**
每个脉冲的中电都以相应的三角波中点为对称，从而大大简化了计算过程。
